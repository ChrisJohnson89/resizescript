...
# Set Redis repository configuration
echo "" > /etc/apt/sources.list.d/redis.list || echo "Redis Not Found."
echo "deb http://archive.ubuntu.com/ubuntu focal main universe" >> /etc/apt/sources.list.d/redis.list || echo "Redis Not Found."
...
if [[ "$SERVER_TYPE" == "Master" ]]; then
    echo "Stopping services on Master server..."
    for service in "${services[@]}"; do
...
elif [[ "$SERVER_TYPE" == "Sql" ]]; then
    echo "Stopping services on SQL server..."
    for service in "nginx" "varnish"; do
        if systemctl stop $service; then
            echo "Stopped $service"
        else
            echo "Failed to stop $service"
    ...
    echo "Stopping services on Node server..."
    ...
    mount | grep "type nfs" | awk '{print $3}' | xargs -I {} umount {} || { echo "Failed to unmount NFS on Node server."; exit 1; }
else
    echo "Invalid server type. Exiting script."
    exit 1
fi
